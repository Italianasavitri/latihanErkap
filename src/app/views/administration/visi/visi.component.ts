...
export class VisiComponent implements AfterViewInit {
  ...
  getVisiData() {
    ...
    this.visiService.getAll().subscribe(resp => {
      this.visiService.getAllVisiAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
            return element.userVisi.id === value.id && element.menuTab.activationCode === 'Y';
          });
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewVisi">').dxButton({
      icon: 'add',
  ...
  searching() {
    this.visiService.getByName(this.search.nama).subscribe(resp => {
      this.visiService.getAllVisiAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
            return element.userVisi.id === value.id;
          });
      ...
      if (this.search.nama !== '') {
        this.gridDataSource = resp.filter(visi => visi.visiName === this.search.nama).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  onDeleteConf() {
    this.visiService.getById(this.target).subscribe(visi => {
      this.visiService.delete(visi.d).subscribe(resp => {
        // this.getVisiData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
